    ...
    .run([
      ...
      function ($templateCache) {
        $templateCache.put('template/treeGrid/treeGrid.html',
          ...
          "       ng-click=\"selectRow($index)\" ng-class=\"'level-' + {{ row.level }} + ($index == selectedRow ? ' info': '')\" class=\"tree-grid-row\">\n" +
      ...
      function ($timeout,
        ...
        return {
          ...
          link       : function (scope, element, attrs) {
            ...
            var sortByRecursive = function(dataObj, field, sortDirection, sortingType) {
                for (var x=0; x<dataObj.length; x++) {
                    if (dataObj[x].children.type !== null && dataObj[x].children.type === "directory") {
                        continue;
                    } else {
                        if (sortDirection === "asc") {
                            dataObj[x].children.sort(sort_by(field, true, sortingType));
                        } else {
                            dataObj[x].children.sort(sort_by(field, false, sortingType));            		
                        }
                        sortByRecursive(dataObj[x].children, field, sortDirection, sortingType);
                    }
                }
            };
            /* sorting methods */
            scope.sortBy = function (col) {
                ...
                sortByRecursive(scope.treeData, col.field, col.sortDirection, col.sortingType);
            ...
            on_treeData_change = function () {
              ...
              add_branch_to_list = function (level, branch, visible) {
                ...
                // Custom edit (START) - Daniele Gazzelloni
                } else if (branch.children.type == "directory") {
                  tree_icon = attrs.iconExpand;  
                // Custom edit (END) - Daniele Gazzelloni  
                } else {
                  if (branch.expanded) {
